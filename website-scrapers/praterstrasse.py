#!/usr/bin/env python3
"""
Praterstrasse Event Scraper
Extracts upcoming events from https://praterstrasse.wien/en/praterstrasse-tickets-9djnDeMk/
and saves them to the Supabase database.

Usage:
    python website-scrapers/praterstrasse.py [--dry-run] [--debug]
"""
...
class PraterstrasseScraper(BaseVenueScraper):
    """Scraper for Praterstrasse Vienna events"""
    ...
    def scrape_events(self) -> List[Dict]:
        """Scrape events from Praterstrasse"""
        ...
        # Praterstrasse ticket page structure
        event_selectors = [
            'div.event-item',
            'article.ticket-item',
            'div[class*="event"]',
    ...
    def _parse_event_item(self, item) -> Optional[Dict]:
        ...
        try:
            ...
            # Extract title
            title_selectors = ['h2', 'h3.event-title', 'h3']
            ...
            # Extract time
            time_elem = item.select_one('span.time, .event-time')
            if time_elem:
                time_text = time_elem.get_text(strip=True)
                event_data['time'] = self.parse_time(time_text)
            
            # Extract event series (KLUBNACHT, GAZE, etc.)
    ...
    def _enrich_from_detail_page(self, event_data: Dict):
        ...
        try:
            ...
            # Extract lineup
            lineup_elem = soup.select_one('.lineup')
            if lineup_elem:
...
def main():
    """CLI entry point"""
    parser = argparse.ArgumentParser(description='Scrape Praterstrasse events')
    parser.add_argument('--dry-run', action='store_true', help='Run without saving to database')
    parser.add_argument('--debug', action='store_true', help='Enable debug output')
    args = parser.parse_args()
    
    scraper = PraterstrasseScraper(dry_run=args.dry_run, debug=args.debug)
    result = scraper.run()
