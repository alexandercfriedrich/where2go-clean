# Ibiza Spotlight Scraper - Comprehensive Event Coverage Roadmap

## 🎯 Objective

Implement a complete overhaul of the Ibiza Spotlight scraper to:
1. **Capture full month of events** by scanning 4 rolling 7-day windows
2. **Fix month boundary issues** (Dec/Jan transitions)
3. **Improve data extraction accuracy** with Ibiza-specific selectors
4. **Add robust error handling** with retry logic
5. **Maximize event discovery** across all date ranges

---

## 📊 Implementation Strategy

### **PHASE 1: Multi-Window Date Range Scanning**

Implement a "rolling window" approach that scrapes 4 consecutive 7-day windows to capture ~30 days of events:

```
Window 1: today -> today+6
Window 2: today+7 -> today+13
Window 3: today+14 -> today+20
Window 4: today+21 -> today+27
```

**This approach:**
- Captures entire month's events
- Naturally handles month boundaries (rolling dates)
- Increases coverage from 7 to ~30 days
- Each window is a complete pass through the calendar
...
Visit each event's detail page for comprehensive data:
...
Update `scrape_events()` to use rolling windows:
        ...
        for card_idx, card in enumerate(event_cards, 1):
            ...
            if event_data and event_data.get('title'):
                ...
                self.log(
                    ...
                    f"{event_data.get('date', '?'):10} | "
                    f"{event_data.get('start_time', ''):5}",
...
- [ ] Add `_generate_rolling_window_urls()` method
  - [ ] Returns exactly 4 URLs
  - [ ] Each window is 7 days
  - [ ] Correct date formatting (DD/MM/YYYY)
  - [ ] Month boundaries handled

- [ ] Add `fetch_page_with_retry()` method
  - [ ] 3 retry attempts
  - [ ] Exponential backoff (1s, 2s, 4s)
  - [ ] Proper error logging

- [ ] Update `_parse_event_card()` method
  - [ ] Extract all 9 fields (title, date, times, venues, artists, price, image, detail_url, description)
  - [ ] Support multiple venues and artists
  - [ ] Deduplicate lists
  - [ ] Handle missing fields

- [ ] Update `_enrich_from_detail_page()` method
  - [ ] Use `fetch_page_with_retry()`
  - [ ] Extract description, image, artists, ticket_url
  - [ ] Graceful failure if page unavailable

- [ ] Update `scrape_events()` method
  - [ ] Call `_generate_rolling_window_urls()`
  - [ ] Process 4 windows sequentially
...
✅ **Complete When:**
1. 4 rolling windows generated correctly
2. ~30 days of event coverage
3. Month boundaries handled seamlessly
4. Retry logic prevents failures
5. All event fields populated
6. No duplicate events
7. Clear logging for each window
8. All tests pass
...
**Before:**
- 7 days of events
- Limited month coverage
- No error recovery
- ~80% data completeness

**After:**
- ~30 days of events (4 windows)
- Full month coverage
- Robust error recovery (3 retries)
