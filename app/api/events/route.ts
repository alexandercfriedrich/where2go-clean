...
export async function POST(request: NextRequest) {
  try {
    ...
    // Wien.at RSS via Hot Cities – Kategorien aus UI → KAT1
    try {
        ...
        if (rssSites.length) {
          ...
          for (const site of rssSites) {
            const initial = await fetchWienAtEvents({
              ...
              fromISO: date,
              toISO: date,
              extraQuery: site.searchQuery || '', // ignoriert KATx wenn viennaKats gesetzt
              viennaKats,
              limit: 500
            });
            rssResults.push(...initial);

            // Fallback: Wenn mit KATs 0 Treffer → nochmal ohne KAT filtern (breiter)
            if (initial.length === 0) {
