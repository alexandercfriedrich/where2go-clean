// Perplexity API integration with multi-query support and rate limiting (updated to dynamic 20 categories + guidance)
...
`;
  ...
  /**
   * Builds a general query prompt with strict JSON schema (dynamic 20 categories)
   */
...
`;
  ...
  private async createQueries(city: string, date: string, categories?: string[]): Promise<string[]> {
    ...
    for (const category of categories) {
      // Accept input as already main category OR fallback
