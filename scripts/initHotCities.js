...
async function saveHotCities(cities) {
  await ensureDataDir();
...
async function initializeHotCities() {
  console.log('Initializing hot cities data...');
  await saveHotCities(initialHotCities);
  console.log(`Created ${initialHotCities.length} hot cities with websites:`);
  initialHotCities.forEach(city => {
    console.log(`  ${city.name}: ${city.websites.length} websites`);
  });
}

// Run if called directly
if (require.main === module) {
  initializeHotCities().catch(console.error);
}

module.exports = { initializeHotCities };