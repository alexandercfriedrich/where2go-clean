-- Migration: Update venue functions to use venue_slug column
-- Description: Update get_venue_with_events and get_top_venues to use stored venue_slug
-- Created: 2025-11-24

-- Update get_venue_with_events to use venue_slug column
...
-- Update get_top_venues to use venue_slug column
...
BEGIN
  ...
  WITH venue_events AS (
    ...
    GROUP BY v.id, v.name, v.address, v.city, v.venue_slug
    HAVING COUNT(*) FILTER (WHERE e.start_date_time >= NOW()) > 0
  )
  SELECT 
    ve.id::UUID,
    COALESCE(ve.venue_slug, slugify(ve.name))::TEXT,
    ve.name::TEXT,
    ve.address::TEXT,
    ve.city::TEXT,
...
COMMENT ON FUNCTION get_venue_with_events(TEXT, TEXT) IS 'Returns venue details with stats and upcoming events by venue slug (stored or generated), optionally filtered by source';
COMMENT ON FUNCTION get_top_venues(TEXT, INTEGER, TEXT) IS 'Returns top venues for a city ranked by upcoming event count, using stored venue_slug, optionally filtered by source';
